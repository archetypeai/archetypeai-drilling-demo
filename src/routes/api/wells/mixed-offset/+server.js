...
export async function GET({ url }) {
	...
	const windowSize = parseInt(url.searchParams.get('windowSize') || '64', 10);
	const numWindows = parseInt(url.searchParams.get('numWindows') || '41', 10);
	...
	try {
		...
		// Read ACTC column only for efficiency
		const actcValues = [];
		...
		// Scan for the section with the best mix
		let bestOffset = 0;
		let bestMix = 0;
		...
		for (let offset = 0; offset + scanSize <= actcValues.length; offset += step) {
			...
			if (bestMix >= 0.45) break; // Good enough, near-perfect balance
		}
